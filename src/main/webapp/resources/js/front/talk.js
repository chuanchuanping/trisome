...
$(function(){
	render();
	var content = "秀念走的第一天，想他，秀念头七，想他！<br/>回复[查看]收取更多精彩内容。";
	content += "<br/>回复[帮助]可以查看所有可用的指令。";
	// 添加公众号的开场白
	appendDialog("talk_recordbox","秀念师兄",content);
	render();
});
...
function send() {
	...
	$.ajax({
		...
		success : function (answer) {
			appendDialog("talk_recordboxme","我",content);
            appendDialog("talk_recordbox","",answer.text);
			if(answer.url!=null){
                appendDialogIamge("talk_recordbox","",answer.url);
            }
			$("#content").val("");
			render();
		},
...
function appendDialog(myClass,name,content) {
    ...
    div += "<div class='user'><img src='" + $("#basePath").val() + "resources/images/thumbs/" + myClass + ".png'/>" + name + "</div>";
...
function appendDialogIamge(myClass,name,content) {
    var div = "";
    div += "<div class='" + myClass + "'>";
    div += "<div class='user'><img src='" + $("#basePath").val() + "resources/images/thumbs/" + myClass + ".png'/>" + name + "</div>";
    div += "<div class='talk_recordtextbg'>&nbsp;</div>";
    div += "<div class='talk_recordtext'>";
    div += "<div ><img src='" + content + "'  height=\"200\" width=\"120\" /></div>";
    div += "<span class='talk_time'>" + getCurrentDate() + "</span>";
    div += "</div>";
    div += "</div>";
    $('#jp-container').children().eq(0).children().eq(0).append(div);
